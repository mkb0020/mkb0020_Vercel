...
# NATURAL BUCKET SEQUENCE PRIORS (BLENDED WITH REAL DATA)
# EACH ENTRY: (NEXT_BUCKET, PRIOR_WEIGHT)
BUCKET_SEQUENCE_PRIOR ={
"long":[("long",0.35 ),("medium",0.50 ),("short",0.15 )],
"medium":[("long",0.20 ),("medium",0.40 ),("short",0.40 )],
"short":[("long",0.10 ),("medium",0.45 ),("short",0.45 )],
}

# PER-BUCKET DURATION-TYPE PRIORS
BUCKET_RHYTHM_PRIOR ={
"long":{"whole":4 ,"dotted-half":3 ,"half":3 ,"dotted-quarter":1 },
"medium":{"half":2 ,"dotted-quarter":3 ,"quarter":4 ,"dotted-eighth":1 },
"short":{"quarter":2 ,"dotted-eighth":2 ,"eighth":5 ,"sixteenth":3 },
}
...
def build_harmony_rules (events ):
    ...
    for e in events :
        if not e ['is_rest']:
            k =(e .get ('measure_number'),round (float (e .get ('beat_position')or 0 ),2 ))
            by_beat [k ].append (e )

    pair_counts =defaultdict (int )
    pair_smooth =defaultdict (float )
    for key ,group in by_beat .items ():
        if len (group )<2 :
            continue 
        sorted_g =sorted (group ,key =lambda e :-(pitch_to_midi (e .get ('pitch',''))or 0 ))
        melody =sorted_g [0 ]
        mel_p =melody .get ('pitch')
        smooth =float (melody .get ('connection_score')or 0.5 )
        for other in sorted_g [1 :]:
            hp =other .get ('pitch')
            if hp and hp !='REST':
                pair_counts [(mel_p ,hp )]+=1 
                pair_smooth [(mel_p ,hp )]+=smooth 

    by_melody =defaultdict (list )
    for (mel ,harm ),cnt in pair_counts .items ():
        vl =voice_leading_penalty (mel ,harm )
        by_melody [mel ].append ({'note':harm ,'raw':cnt *vl })

    harmony_rules ={}
    for mel ,candidates in by_melody .items ():
        total =sum (c ['raw']for c in candidates )
        if total >0 :
            for c in candidates :
                c ['weight']=round (c ['raw']/total ,6 )
        harmony_rules [mel ]=[{'note':c ['note'],'weight':c .get ('weight',0 )}
        for c in candidates ]

    pc_counts =defaultdict (lambda :defaultdict (int ))
    for (mel ,harm ),cnt in pair_counts .items ():
    ...
    for mel_pc ,harm_map in pc_counts .items ():
        total =sum (harm_map .values ())
        pc_rules [mel_pc ]=[
        {"note":h ,"weight":round (cnt /total ,6 )}
        for h ,cnt in sorted (harm_map .items (),key =lambda x :-x [1 ])
        ]
...
def build_smoothness_rules (events ):
    ...
    for e in events :
        ...
        if pd_ and cd and cs is not None :
            rhythm_flow_map [pd_ ][cd ].append (cs )
